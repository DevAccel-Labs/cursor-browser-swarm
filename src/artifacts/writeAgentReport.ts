...
export function renderAgentReport(report: AgentRunReport): string {
  ...
  return [
    ...
    "",
    "## Mission",
    `Directive: ${report.assignment.directive.id} (${report.assignment.directive.label})`,
    `Directive instructions: ${report.assignment.directive.instructions}`,
    `Destructive actions allowed: ${report.assignment.directive.allowDestructiveActions ? "yes" : "no"}`,
    "",
    routes,
    "",
    "## Evidence",
